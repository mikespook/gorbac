...
type AssertionFunc func(string, Permission, *RBAC) bool
...
func New() *RBAC {
	return NewWithFactory(NewStdRole)
}
...
func (rbac *RBAC) Set(name string, permissions []Permission, parents []string) {
	rbac.mutex.Lock()
...
// and keep orignals.
func (rbac *RBAC) Add(name string, permissions []Permission, parents []string) {
	rbac.mutex.Lock()
	defer rbac.mutex.Unlock()
	role := rbac.getRole(name)
	for _, p := range permissions {
...
func (rbac *RBAC) IsGranted(name string, permission Permission,
